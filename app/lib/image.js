...
export async function createThumbnail(file) {
  const acceptedTypes = ["image/webp", "image/png", "image/jpg", "image/jpeg"];
  const bucketLimit = 0.95;

  // Check file and file type
  if (!file || acceptedTypes.indexOf(file.type) === -1) {
    alert(
      `Please an image file of the following types: ${acceptedTypes.map((item) => item.split("/")[1]).join(", ")}.`,
    );
    return;
  }
  ...
  if (imageSizeMB > bucketLimit) {
    alert(
      `Image after compression (${imageSizeMB.toFixed(2)} MB) is greater than the maximum transferrable size (${bucketLimit} MB). Please select a smaller image.`,
    );
    return;
  }
