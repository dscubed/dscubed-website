...
export default function InfiniteBanner({
  ...
  const parentRef: React.RefObject<HTMLDivElement | null> =
  ...
  const startAnimation = () => {
    ...
    intervalId.current = setInterval(() => {
      refs.forEach(
        (ref: React.RefObject<HTMLElement | null>, index: number) => {
  ...
  return (
    <div className="h-max w-screen">
      <div
        ref={parentRef}
        className="relative h-20 transition-opacity duration-500 opacity-0"
      >
